...
fun dotBeforeNumber (n: List<String>): String {
    return n.joinToString("", "1.")
}
...
fun hasAnagrams(words: List<String>): Boolean {
    for (i in words.size downTo 1) {
        if (wordToCharSet(words[i]) != wordToCharSet(words[i-1])) return false
    }
    return true
}
