...
data class Circle(val center: Point, val radius: Double) {
    ...
    fun distance(other: Circle): Double {
        ...
        return if (centerDistance <= radius + other.radius) 0.0
        else centerDistance - (radius + other.radius)
    }
...
fun diameter(vararg points: Point): Segment {
    ...
    for (point1 in points) {
        for (point2 in points) {
            if (point1.distance(point2) >= maxLength) {
                maxLength = point1.distance(point2)
                diameter = Segment(point1, point2)
            }
        }
    }
    return diameter
...
class Line private constructor(val b: Double, val angle: Double) {
    ...
    fun crossPoint(other: Line): Point {
        ...
        val y: Double
        if (angle != PI / 2) {
            x = (other.b - b * (cos(other.angle) / cos(angle))) / (sin(angle) * cos(other.angle) / cos(angle) - sin(
                other.angle
            ))
